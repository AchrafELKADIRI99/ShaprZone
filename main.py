...
def gen1(url, student_class):
    ...
    print('liste student :  ')
    print(student_data)
    # get d known face and encodings
    ...
    while camera_status == 1:
        ...
        if (Scene.detectChange(img) == True):
            ...
            for encoding in encodings:
                # comparaison of faces nd stock it in matches
                matches = pool2.apply_async(recogFace, (data, encoding,)).get()
                name = "Unkown_"

                # check if matched
                if True in matches:
                    # get indexes of matches then count matches

                    matchedIds = [i for (i, b) in enumerate(matches) if b]

                    counts = {}
                    counts.clear()
                    # loop on recognized faces
                    for i in matchedIds:
                        name = data["names"][i]
                        print('------------name************')
                        print(name)
                    ...
                    if (name not in Attendees_Names):
                        ...
                        for y in student_data:
                            ...
                            if nom[0] in y:
                                print(
                                    '---------------------daaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaate-----------------------')
                                print(y[1])
...
def start_video_py(cam_type, student_class):
    ...
    for each in y:
        print('coco')
        # Convert bytes to base64 encoded so json can be passed
...
def capture_photo_py(url):
    ...
    y = gen(url)
    for each in y:
        # Convert bytes to base64 encoded for json...
        blob = base64.b64encode(each)
        blob = blob.decode("utf-8")
...
def submit_live_attendance(stu_id, student_class, date):
    ...
    # insert data to db
    print("this isssssssssssss submit live att")
    print(stu_id)
    print(stu_id)
    print("this isssssssssssss submit live att")
...
def submit_student_data(stu_id, fullname, student_class, session):
    try:
        print("here its submit stuuuuuuuuuuuuuud")
        print(stu_id)
        print(fullname)
        print(student_class)
        print(session)
        print("here its submit stuuuuuuuuuuuuuud eeeend")
