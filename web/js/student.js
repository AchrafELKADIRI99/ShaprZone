...
function startVideoForCapture() {
  ...
  if (studentId == "") {
    ...
    document.getElementById("photo-capture-stop").classList.remove("d-none");
    document.getElementById("photo-capture").classList.remove("d-none");
    ...
    var f = document.getElementById("photo-capture").classList.add("d-block");
    alert(f);
...
function capturePhoto() {
  ...
  if (studentId == "") {
    ...
    eel.save_photo(studentId, studentName);
  }
}
...
function showCapturePhoto(capturedPhoto) {
  document.getElementById("captured-photo").innerHTML +=
    ...
    capturedPhoto +
    "'style='height:50px;width:50px;' class='col-sm-4 mb-2'>";
}
...
function submitStudentData() {
  ...
  if (stu_id == "" || fullname == "" || student_class == "" || session == "") {
    ...
    document.getElementById("message").innerHTML =
      "<p class='text-success'>Saving student...</p>";
...
function setTableData(studentId, name, student_class, session) {
  ...
  document.getElementById("student-data-table").innerHTML +=
    ...
    "</td><td>" +
    session +
    "</td><td><a href='#' onclick=" +
    onclicktask +
    "><i class='fas fa-trash text-danger'> Delete</i></a></td></tr>";
}
